...
class ECISpider(scrapy.Spider):
    ...
    def parse_constituency(self,response):
        ...
        for constituency_url in constituency_urls:
            absoulte_constituency_url = urljoin(response.url,constituency_url)
    ...
    def parse_candidate(Self,response):
        ...
        file_exists = os.path.isfile('prac.csv')

        with open('prac.csv',mode='a',newline='') as csv_file:
            fieldnames = ['ref','wonstatus','totavotes','winvotes','name','partyname']
            writer = csv.DictWriter(csv_file, fieldnames=fieldnames)

            if file_exists or csv_file.tell ==0:
                writer.writeheader()
